...
std::unique_ptr<IDataCreator> FilesystemTableCreator::CreateTable(std::string table_name) {
  ...
  std::ofstream file(db_name_ + "/" + table_name + ".csv");
  return std::make_unique<FilesystemDataCreator>(db_name_, table_name);
}

ITableCreator& FilesystemTableCreator::DeleteTable(std::string table_name) {
  if (!std::ifstream(db_name_ + "/" + table_name + ".csv")) {
